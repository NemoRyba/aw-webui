...
div
            ...
            b-button-toolbar.float-right
              b-button-group(size="sm", class="mx-1")
                b-button(variant="primary", :to="'/buckets/' + data.item.id")
                  ...
                  | {{ $tr('Open') }}
                b-dropdown(variant="outline-secondary", size="sm", :text="$tr('More')")
                  ...
                  b-dropdown-item(
                             ...
                             :title="$tr('Export bucket as JSON')",
                             variant="secondary")
                      icon(name="download")
                      | {{ $tr('Export bucket as JSON') }}
                  b-dropdown-item(
                              @click="export_csv(data.item.id)",
                             :title="$tr('Export events as CSV')",
                             variant="secondary")
                      icon(name="download")
                  ...
                  b-dropdown-item-button(@click="openDeleteBucketModal(data.item.id)",
                           ...
                           variant="danger")
                    | #[icon(name="trash")] {{ $tr('Delete bucket') }}
  ...
  b-modal(id="delete-modal", :title="$tr('Danger!')", centered, hide-footer)
    | {{ $tr('Are you sure you want to delete bucket') }} "{{delete_bucket_selected}}"?
    br
    br
    b {{ $tr('This is permanent and cannot be undone!') }}
    hr
    div.float-right
  ...
  h3 {{ $tr('Import and export buckets') }}

  b-card-group.deck
    b-card(:header="$tr('Import buckets')")
      b-alert(v-if="import_error" show variant="danger" dismissable)
        | {{ import_error }}
      b-form-file(v-model="import_file"
                  :placeholder="$tr('Choose or drop a file here...')"
                  :drop-placeholder="$tr('Drop file here...')")
      // TODO: This spinner could be placed in a more suitable place
      div(v-if="import_file" class="spinner-border" role="status")
      span
        | {{ $tr('A valid file to import is a JSON file from either an export of a single bucket or an export from multiple buckets.') }}
        | {{ $tr('If there are buckets with the same name the import will fail.') }}
    b-card(:header="$tr('Export buckets')")
      b-button(:href="$aw.baseURL + '/api/0/export'",
               :download="'aw-bucket-export.json'",
               :title="$tr('Export all buckets as JSON')",
               variant="outline-secondary")
        icon(name="download")
        | {{ $tr('Export all buckets as JSON') }}
...
export default {
    ...
    import_file: async function (_new_value, _old_value) {
      if (this.import_file != null) {
        ...
        try {
          ...
          this.import_error = this.$tr('Import failed, see aw-server logs for more info');
        }
  ...
  methods: {
    bucketRows(device) {
      return device.buckets.map(bucket => {
        const identity = getBucketIdentity(bucket);
        return {
          ...bucket,
          username: identity.username,
          session_id: identity.sessionType
    ...
    runChecks: function (device) {
      const checks = [
        {
          msg: () => {
            return this.$tr(`Device known by several hostnames: ${device.hostnames}`);
          },
          failed: () => device.hostnames.length > 1,
        },
        {
          msg: () => {
            return this.$tr(`Device known by several IDs: ${device.device_ids}`);
          },
